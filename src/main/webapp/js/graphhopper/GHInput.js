...
GHInput.isFile = function (value) {
    var stringValue = Object.prototype.toString.call(value);
    return (stringValue.toLowerCase() === "[object file]");
};
...
GHInput.prototype.set = function (strOrObject) {
    ...
    if (GHInput.isObject(strOrObject)) {
        ...
        }else if (GHInput.isFile(strOrObject)){
        	Papa.parse(strOrObject, {
                download: true,
                complete: function(results) {
                    var data = results.data, html;
                  
                        var item = data[0];
                        this.lat = round(parseFloat(item[0]));
                        this.lng = round(parseFloat(item[1]));
